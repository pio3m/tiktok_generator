...
from moviepy.editor import (
    ...
    concatenate_videoclips, ColorClip, CompositeAudioClip, ImageClip
)
...
background_video_path = "../data/video.mp4"  # <-- Zamiast obraz.png
...
def get_zoom_bg(duration):
    clip = VideoFileClip(background_video_path).subclip(0, duration).resize(height=1920)
    return clip.set_duration(duration).set_fps(FPS).resize(lambda t: 1 + 0.01 * t)
...
# --- KLIPY ---
intro_duration = voice.duration
intro = CompositeVideoClip([
    get_zoom_bg(intro_duration),
    get_avatar(intro_duration),
    get_intro_text("Zobacz uważnie ten kadr...", intro_duration)
], size=VIDEO_SIZE)
